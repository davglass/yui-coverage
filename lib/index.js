...
for (var i in meta[moduleName]) {
    switch (i) {
        case 'use':
            meta[moduleName].use.forEach(function(k) {
                mods[k] = true;
            });
            break;
        case 'submodules':
        case 'plugins':
            Object.keys(meta[moduleName][i]).forEach(function(k) {
                mods[k] = true;
            });
            break;
    }
...
module.exports = {
    start: function() {
        console.log('Starting server: http://127.0.0.1:3000/'.magenta);
